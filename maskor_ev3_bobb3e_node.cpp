...
void calc_odometry() {
  ...
  std::cout << "wheelradius: " << wheelradius  << std::endl;
    std::cout << "theta_curr: " << theta_curr  << std::endl;
    ...
    std::cout << " y_curr: " << y_prev  << std::endl;
  ...
  const double l = 0.2;
  theta_curr = theta_prev +((2*M_PI)/360)*(dl - dr) / l;
  x_curr = x_prev + (dr + dl) / 2 * cos(theta_curr);// + ((2*M_PI*wheelradius/360)/axisDistance);
  y_curr = y_prev + (dr + dl) / 2 * sin(theta_curr);// + ((2*M_PI*wheelradius/360)/axisDistance);
